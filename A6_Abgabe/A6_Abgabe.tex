...
Der Dekoder arbeitet auf dem unteren Nibble (Bit 3..0) des 8-Bit-Registers, da die Hex-Literale \texttt{x"0"} bis \texttt{x"9"} nur 4 Bit breit sind und ein Vergleich mit 8-Bit-Vektoren in VHDL eine Längen-Diskrepanz erzeugen würde. Bit 7 des Registers ist für die spätere Steuerung des Dezimalpunkts (DP) vorgesehen, wird aktuell jedoch nicht genutzt (\texttt{point\_status} ist auf '0' initialisiert).
...
Die Simulation bestätigt die korrekte Funktion des Wishbone-Handshakes, der Adressdekodierung und des kombinatorischen Dekoders. Die Ausgänge \texttt{sevensegment\_o} zeigen die in Tabelle~\ref{tab:decoder_values} spezifizierten Muster, wobei die Common-Anode-Logik (aktiv-LOW) berücksichtigt werden muss.
...
%--------------------------------------------------------------------
\subsection{Ergebnisse im Portfolio}

Die Umsetzung der Sieben-Segment-Ansteuerung demonstriert den vollständigen Entwicklungszyklus eines SoPC-Moduls: Von der VHDL-Implementation über die Bus-Integration und Software-HAL bis zur Simulation und Inbetriebnahme an der realen Hardware. Die konsequente Nutzung des Wishbone-Bus-Protokolls ermöglicht eine modulare Erweiterung des Systems um weitere Peripherie, wie sie im weiteren Kursverlauf (WS2812-Display, Game of Life) erforderlich ist.