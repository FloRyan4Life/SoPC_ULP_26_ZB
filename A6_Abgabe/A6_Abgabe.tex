...
Der Ausgang \texttt{sevensegment\_o} ist als \textbf{Common-Kathode} ausgelegt: Ein Segment leuchtet, wenn der entsprechende Ausgang auf \texttt{'0'} (LOW) liegt. Die Zuordnung der Segmente erfolgt gemäß Datenblatt des Kingbright SC03-12EWA (vgl. Script S. 22):
...
\begin{table}[H]
\centering
\caption{7-Segment-Dekodierung für Ziffern 0--9 (Common-Kathode, aktiv-LOW)}
...
\end{tabular}
\\[4pt]
{\small Anmerkung: DP = 1 bedeutet \textit{aus} (Common-Kathode, \texttt{not point\_status} mit \texttt{point\_status = '0'})}
\end{table}
...
\begin{itemize}
    ...
    \item \textbf{Zeile 76:} Der Ausgang wird zu \texttt{seg\_pattern \& (not point\_status)} zusammengesetzt. Aktuell ist \texttt{point\_status} konstant '0', sodass DP immer ausgeschaltet bleibt (für Common-Kathode: \texttt{not '0' = '1'} = aus).
\end{itemize}
...
Die Simulation bestätigt die korrekte Funktion der Adressdekodierung und des kombinatorischen Dekoders. Die Ausgänge \texttt{sevensegment\_o} zeigen die in Tabelle~\ref{tab:decoder_values} spezifizierten Muster, wobei die Common-Kathode-Logik (aktiv-LOW) berücksichtigt werden muss.
...
Die Pin-Zuweisung erfolgt über die \texttt{iceduino.pcf} für freie PMOD- 
oder Arduino-Header-Pins. Die Sieben-Segment-Anzeige vom Typ Kingbright 
SC03-12EWA wird mit Vorwiderständen (ca. 220--330 Ohm pro Segment) an die
FPGA-Ausgänge angeschlossen, um den Strom zu begrenzen. Die Common-Kathode
wird an 0V/GND gelegt, die einzelnen Segmente über die aktiven-HIGH-Ausgänge des FPGA geschaltet.
Zur Ansteuerung wurden die Arduino-Header Pins genutzt. Um die Header zu aktivieren, mussten die zwei $OE5J$
und $OE6J$ Signale auf HIGH gezogen
werden.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth, angle=-90]{A6_Abgabe/resources/IMG_1899.jpeg}
\caption{Foto des Aufbaus der realen Hardware: FPGA-Board mit angeschlossener 7-Segment-Anzeige. Die Anzeige ist über Vorwiderstände mit den FPGA-Ausgängen verbunden, die Common-Kathode liegt an GND.}
